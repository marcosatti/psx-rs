...
pub(crate) fn initialize_video_backend<'a: 'b, 'b>(window: &'a Window) -> VideoBackend<'a, 'b> {
    ...
    unsafe {
        ...
        window
            .gl_make_current(OPENGL_CONTEXT.as_ref().unwrap())
            .unwrap();
    }
    ...
    let opengl_vendor_string = unsafe {
        std::ffi::CStr::from_ptr(glGetString(GL_VENDOR as GLenum) as *const i8)
            .to_string_lossy()
            .into_owned()
    };
    let opengl_version_string = unsafe {
        std::ffi::CStr::from_ptr(glGetString(GL_VERSION as GLenum) as *const i8)
            .to_string_lossy()
            .into_owned()
    };
    let opengl_renderer_string = unsafe {
        std::ffi::CStr::from_ptr(glGetString(GL_RENDERER as GLenum) as *const i8)
            .to_string_lossy()
            .into_owned()
    };
    unsafe {
    ...
    VideoBackend::Opengl(opengl::BackendParams {
        context: BackendContext::new(
            Box::new(opengl_acquire_context),
            Box::new(opengl_release_context),
        ),
    })
}
...
pub(crate) fn initialize_audio_backend<'a: 'b, 'b>() -> AudioBackend<'a, 'b> {
    if !MUTED {
        ...
        unsafe {
            alListenerfv(
                AL_ORIENTATION as ALenum,
                [0.0, 0.0, -1.0, 0.0, 1.0, 0.0].as_ptr(),
            )
        };
        let openal_vendor_string = unsafe {
            std::ffi::CStr::from_ptr(alGetString(AL_VENDOR as ALenum))
                .to_string_lossy()
                .into_owned()
        };
        let openal_version_string = unsafe {
            std::ffi::CStr::from_ptr(alGetString(AL_VERSION as ALenum))
                .to_string_lossy()
                .into_owned()
        };
        let openal_renderer_string = unsafe {
            std::ffi::CStr::from_ptr(alGetString(AL_RENDERER as ALenum))
                .to_string_lossy()
                .into_owned()
        };
        log::info!(
        ...
        AudioBackend::Openal(openal::BackendParams {
            context: BackendContext::new(
                Box::new(openal_acquire_context),
                Box::new(openal_release_context),
            ),
        })
    } else {
        AudioBackend::None
...
pub(crate) fn initialize_cdrom_backend<'a: 'b, 'b>() -> CdromBackend<'a, 'b> {
    ...
    unsafe {
        ...
        LIBMIRAGE_CONTEXT =
            g_object_new(mirage_context_get_type(), std::ptr::null()) as *mut MirageContext;
        assert!(!LIBMIRAGE_CONTEXT.is_null());
    }

    let libmirage_acquire_context = || unsafe { &LIBMIRAGE_CONTEXT };
    let libmirage_release_context = || {};
    let libmirage_version_string = unsafe {
        std::ffi::CStr::from_ptr(mirage_version_long)
            .to_string_lossy()
            .into_owned()
    };
    ...
    CdromBackend::Libmirage(libmirage::BackendParams {
        context: BackendContext::new(
            Box::new(libmirage_acquire_context),
            Box::new(libmirage_release_context),
        ),
    })
}

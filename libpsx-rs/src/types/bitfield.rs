...
impl Bitfield {
    #[must_use]
    pub(crate) const fn new(start: usize, length: usize) -> Bitfield {
        Bitfield {
            start,
            length,
        }
    ...
    #[must_use]
    pub(crate) fn unshifted_mask<T>(&self) -> T
    where T: Shl<usize, Output = T> + Sub<T, Output = T> + One {
        (T::one() << self.length) - T::one()
    }

    #[must_use]
    pub(crate) fn shifted_mask<T>(&self) -> T
    where T: Shl<usize, Output = T> + Sub<T, Output = T> + One {
        Self::unshifted_mask::<T>(self) << self.start
    }

    #[must_use]
    pub(crate) fn extract_from<T>(&self, value: T) -> T
    where T: Shl<usize, Output = T> + Sub<T, Output = T> + One + Shr<usize, Output = T> + BitAnd<T, Output = T> {
        (value & self.shifted_mask()) >> self.start
    }

    #[must_use]
    pub(crate) fn insert_into<T>(&self, destination: T, source: T) -> T
    ...
    #[must_use]
    pub(crate) fn acknowledge_mask<T>(&self, value: T) -> T
    where T: Shl<usize, Output = T> + Sub<T, Output = T> + BitAnd<T, Output = T> + Not<Output = T> + One {
        !(value & self.shifted_mask())
    }

    #[must_use]
    pub(crate) fn acknowledge<T>(&self, value: T, acknowledge_value: T) -> T
    where T: Shl<usize, Output = T> + Sub<T, Output = T> + BitAnd<T, Output = T> + Not<Output = T> + One {
        value & self.acknowledge_mask(acknowledge_value)
    }

    #[must_use]
    pub(crate) fn copy<T>(&self, destination: T, source: T) -> T
    where T: Shl<usize, Output = T> + Sub<T, Output = T> + One + Shr<usize, Output = T> + BitAnd<T, Output = T> + BitOr<T, Output = T> + Not<Output = T> {

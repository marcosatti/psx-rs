...
static TRACE_INSTRUCTIONS_LENGTH: usize = 10;

pub unsafe fn trace_instructions_at_pc(state: &State) {
    ...
    let u8_length2 = (TRACE_INSTRUCTIONS_LENGTH as u32 / 2) * INSTRUCTION_SIZE;
    ...
    let trace_info = dump_instructions(u8_start as u32, slice);
    trace!("Instruction dump at 0x{:08X}:\n{}", pc, &trace_info);
}

fn dump_instructions(base_address: u32, raw_instructions: &[u8]) -> String {
    let cs = Capstone::new()
        .mips()
    ...
    for (i, value) in instructions.iter().enumerate() {
        if i == (TRACE_INSTRUCTIONS_LENGTH / 2) {

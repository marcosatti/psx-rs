...
pub(crate) fn handle_command(state: &State, controller_state: &mut ControllerState, cdrom_backend: &CdromBackend) -> ControllerResult<()> {
    ...
    let parameters_needed = (handler.0)(command_iteration);
...
fn get_handler_fn(command_index: u8) -> ControllerResult<(LengthFn, HandlerFn)> {
    match command_index {
        ...
        0x08 => Ok((command_impl::command_08_length, command_impl::command_08_handler)),
        ...
        0x1A => Ok((command_impl::command_1a_length, command_impl::command_1a_handler)),
